...
def Test(lib, seed=0, size=10, rounds=10, verbose=False):
    ...
    for i in range(1,size):
        ...
        try:
            print("Inserting node with data {}".format(n))
            tree = tree.insert(n)
        except:
            print(c)
            if verbose:
        ...
        print("Getting height of tree with data {}".format(tree.data))
        tH = tree.getHeight()
        if CheckHeight(tree) != tH:
            if verbose:
                print("Error: Tree height calculation incorrect")

                print(f"HEIGHT SHOULD BE: {CheckHeight(tree)} BUT IS {tH}")
        ...
        if tH != h:
            ...
            print(f"CORRECT: Tree height correct: {h} is {tH}")
